...
from soli.utilities.math.general import getFactors, getGoldenestFactor, getDivisorsAndRemainders
...
def getSquarePackedArea(plots, radius, walkwayWidth):
    ...
    return plots.rows * (plots.cols + (plots.remainder > 0)) * areaOfOneCrop + walkwayWidth * plots.perimeter + 4 * walkwayWidth ** 2
...
def getTrianglePackingAreaData(numberOfCrops, radius):
    ...
    trianglePacking["length"] = radius * 2 * trianglePacking.longRow + (1 - (trianglePacking.longRow - trianglePacking.shortRow)) * radius
